#!/usr/bin/env bash
# set-status.sh — write the `check-my-vibe-protection` commit status on a PR head commit.
#
...
_env_skill="${CHECKMYVIBE_SKILL:-}"
_env_context="${CHECKMYVIBE_CONTEXT:-}"
_env_docs="${CHECKMYVIBE_DOCS_URL:-}"

# shellcheck disable=SC1090
[[ -f "$CONFIG_FILE" ]] && source "$CONFIG_FILE"

[[ -n "$_env_skill" ]]   && CHECKMYVIBE_SKILL="$_env_skill"
[[ -n "$_env_context" ]] && CHECKMYVIBE_CONTEXT="$_env_context"
[[ -n "$_env_docs" ]]    && CHECKMYVIBE_DOCS_URL="$_env_docs"

SKILL="${CHECKMYVIBE_SKILL:-check-my-vibe}"
CONTEXT="${CHECKMYVIBE_CONTEXT:-check-my-vibe-protection}"
...
Writes the `check-my-vibe-protection` commit status on a pull request's head commit.
...
Config (.checkmyvibe/config, overridable by env):
  CHECKMYVIBE_SKILL       slash command shown in the unblock message (default: check-my-vibe)
  CHECKMYVIBE_CONTEXT     status check name; must match branch protection (default: check-my-vibe-protection)
  CHECKMYVIBE_DOCS_URL    the "Details" link on the status
