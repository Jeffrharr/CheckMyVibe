#!/usr/bin/env bash
# gate-init.sh — vendor the CheckMyVibe gate (scripts, config, workflow) into a repo.
#
# Self-locates relative to its own path via BASH_SOURCE, so it works whether run
# from a toolkit clone (scripts/gate-init.sh) or from inside a Claude Code plugin
# cache (plugins/checkmyvibe/scripts/gate-init.sh, a symlink back to this file).
# Does not touch skills — those are installed separately (by install-into.sh, or
# already present via /plugin install).
set -euo pipefail
...
usage() {
  cat <<'EOF'
Usage: gate-init.sh <path-to-target-repo>

Vendors into the target repo:
  • .checkmyvibe/set-status.sh             — the author status writer
  • .checkmyvibe/set-review-status.sh      — the per-reviewer status writer
  • .checkmyvibe/config                    — config template (skipped if one exists)
...
Vendored the CheckMyVibe gate into: $TARGET
...
Next steps (manual):
  1. Commit the gate workflow (.github/workflows/checkmyvibe-gate.yml). The
     .checkmyvibe/ dir is gitignored as local tooling — each developer vendors it.
